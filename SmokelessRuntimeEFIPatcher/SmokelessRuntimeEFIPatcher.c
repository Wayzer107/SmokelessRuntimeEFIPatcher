...
void
LogToFile(EFI_FILE *LogFile, char *String)
{
...
SREPEntry(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  while (curr_pos < ConfigDataSize) {
    ...
    if ((Prev_OP->ID == LOAD_FS || Prev_OP->ID == LOAD_FV || Prev_OP->ID == LOADED) &&
        Prev_OP->Name == 0)
    {
    ...
    if (Prev_OP->ID == PATCH && Prev_OP->PatterType != 0 && Prev_OP->ARG3 == 0) {
      if (Prev_OP->PatterType == OFFSET || Prev_OP->PatterType == REL_NEG_OFFSET ||
          Prev_OP->PatterType == REL_POS_OFFSET)
      {
